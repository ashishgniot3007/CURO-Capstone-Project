...
export default function Booking() {
  ...
  /*
   * BLOCK GUESTS AND PROVIDERS
   *
   * Booking is only available to authenticated patients.
   */
  if (!token) {
  ...
  if (loading) {
    return (
      <div className="container-page py-14 text-center text-ink-soft">
  ...
  async function handleContinue() {
    ...
    if (step === 0) {
      ...
      try {
        ...
        setBookingError(
          err.message ||
            "This slot is already booked or taken. Please select another slot."
        );
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (verifiedBooking.status === "CONFIRMED") {
        ...
        throw new Error(
          `Booking status is ${verifiedBooking.status}, expected CONFIRMED.`
        );
      }
    } catch (err) {
      setBookingError(
  ...
  async function handleSimulateFail() {
    ...
    try {
      ...
      setBookingError(
        "Payment simulated failure. Booking cancelled and slot released."
      );
      ...
      <div className="mt-8 rounded-xl2 border border-line bg-white p-6 sm:p-8">
        ...
        {step === 0 && (
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              with {doctor.name} ({doctor.speciality})
            </p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {availableSlots.length > 0 ? (
                ...
                <div className="col-span-full py-6 text-center text-sm text-ink-soft">
                  No slots currently available. Please check
                  back later.
                </div>
              )}
            </div>
        ...
        {step === 1 && selectedSlot && (
          <div>
            ...
            <div className="mt-6 space-y-3 rounded-xl border border-line bg-paper-dim p-5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-sm text-ink">
                  {formatSlotString(selectedSlot.startTime)}
                </span>
              </div>
            </div>
          </div>
        ...
        {step === 2 && (
          <div>
            ...
            <p className="mt-1 text-sm text-ink-soft">
              A simulated interface to process payment.
            </p>

            {paymentUrl && (
              <div className="mt-4 rounded-lg border border-teal-100 bg-teal-50 p-3 font-mono text-xs text-teal-700">
                Payment Link:{" "}
                <a
                  href={paymentUrl}
                  target="_blank"
            ...
            <div className="mt-6 space-y-2">
              {["UPI", "Card", "Net banking"].map((p, i) => (
                <label
                  key={p}
                  className="flex items-center gap-2 rounded-lg border border-line px-3.5 py-2.5 text-sm has-[:checked]:border-teal-500 has-[:checked]:bg-teal-50"
                >
                  <input
                    type="radio"
                    name="pay"
            ...
            <div className="mt-6 text-center">
              <button
                ...
                disabled={apiLoading}
                className="font-medium text-xs text-pulse hover:underline"
              >
        ...
        {step === 3 && (
          <div className="py-6 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm text-ink-soft">
              Your appointment with {doctor.name} is
        ...
        {!isLast && (
          <div className="mt-8 flex justify-end border-t border-line pt-6">
            <Button
              ...
              disabled={apiLoading}
              onClick={
                step === 2 ? handlePay : handleContinue
              }
            >
              {apiLoading
                ? "Processing..."
