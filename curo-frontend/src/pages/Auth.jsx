...
export default function Auth({ mode = "login" }) {
  ...
  function validateForm() {
    ...
    if (role === "Doctor" && !isLogin && !formData.medicalRegistrationNumber.trim()) {
    ...
    try {
      if (isLogin) {
        ...
        const response = await registerUser({
          ...
          password: formData.password,
          role,
          medicalRegistrationNumber: role === "Doctor" ? formData.medicalRegistrationNumber : undefined,
        });

        if (response.success) {
          localStorage.setItem(
            `user_profile_${formData.email.toLowerCase()}`,
            JSON.stringify({
              name: formData.name,
              role: role,
              medicalRegistrationNumber: role === "Doctor" ? formData.medicalRegistrationNumber : undefined,
  ...
  return (
    <div className="container-page flex min-h-[calc(100vh-4rem)] items-center justify-center py-14">
      <div className="w-full max-w-md">
        ...
        <div className="mt-6 flex rounded-full border border-line bg-white p-1">
          {ROLES.map((r) => (
            <button
              key={r}
              type="button"
              ...
              className={`flex-1 rounded-full py-2 text-sm font-medium transition-colors ${
                role === r ? "bg-teal-500 text-white" : "text-ink-soft hover:text-ink"
              }`}
            >
              {r}
            </button>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 rounded-xl2 border border-line bg-white p-6 shadow-card">
          ...
          >
            {loading ? (
              ...
              <span>{isLogin ? "Log in" : `Create ${role.toLowerCase()} profile`}</span>
            )}
          </Button>
        </form>
